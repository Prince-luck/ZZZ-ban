...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "Prince-luck"],
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "WOLF") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "WOlf") => {
...
app.listen(randomPort, () =>
  console.log(
    "ZZZ-ban Server listening on http://localhost:" + randomPort + "/ "
  )
);
...
async function MakeSession(
  ...
  const officialRepoPath = "/Prince-luck/";
